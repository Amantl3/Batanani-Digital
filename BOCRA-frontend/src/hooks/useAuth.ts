...
export function useAuth() {
	...
	const loginMutation = useMutation({
		mutationFn: (creds: LoginCredentials) => {
			console.log('[useAuth] login() called with email:', creds.email);
			return authService.login(creds);
		},
		onSuccess: ({ user, accessToken }) => {
			console.log('[useAuth] Login successful for user:', user.email);
			store.setUser(user);
		...
		onError: (err: any) => {
			...
			console.error('[useAuth] Error detail:', err?.detail);
			console.error('[useAuth] Full error object:', JSON.stringify(err));
			toast.error(
